...
# from baseline_ela import compute_ela
import numpy as np
...
class _Program(object):
    ...
    @staticmethod
    def get_max_depth(program):  # 求树的深度
        ...
        #     if i == 0 and not isinstance(node, _Function):
        #         print(f"error!")
        #         print(node)
        #         print(str(program))
        #         print(str(old_program))
        #         print(str(donor))
        #         print(f"old program : {start} to {end}")
        #         print(f"donor program : {donor_start} to {donor_end}")
        #         print(str(old_program[:start]),str(old_program[end:]))
        #         print(str(donor[donor_start:donor_end]))
        #         raise
        #     if isinstance(node, _Function):
    ...
    def raw_fitness(self, X, y, sample_weight):
        ...
        if np.isnan(y_pred).any() or np.isinf(y_pred).any():
            return penalty
        # 常数函数也无法计算ela
        if np.max(y_pred) - np.min(y_pred) < 1e-8:
            return penalty
        # 计算个体的ela特征
        _,ela_vector = compute_ela(X,y_pred)
        self.ela_vector = ela_vector
        # zscore标准化
        normalize_ela = (ela_vector - self.mean_vec)/self.std_vec
        # PCA降维(1，8)x (8,2)
        self.coordi_2D = np.matmul(normalize_ela.reshape(1,-1),self.pca_scaler)

        # check是否有非法值
        if np.isnan(self.coordi_2D).any() or np.isinf(self.coordi_2D).any():
            return penalty
                
        raw_fitness1 = self.metric(self.target_vec, self.coordi_2D, sample_weight)

        dimension_penalty = self.check_dimension_used()
        
        raw_fitness = raw_fitness1 + np.power(2,dimension_penalty)
        
        return raw_fitness

    def check_dimension_used(self):
        node_list = copy.deepcopy(self.program)
        # 整数节点是变量，数字代表index
        # float代表常数节点
        used_dimensions = set()
        for node in node_list:
            if isinstance(node, int) and 0 <= node < self.n_features:
                used_dimensions.add(node)
        return int(self.n_features - len(used_dimensions))
    ...
    def crossover(self, donor, random_state):
        ...
        while not is_get:
            ...
            # 需要检查交叉后深度是否超过限制
            new_program = (self.program[:start] +
                    donor[donor_start:donor_end] +
                    self.program[end:])
            
            if self.get_max_depth(new_program) > 10:
                counter += 1
                if counter >= 6:  # 尝试次数超过5次就停止，不交叉突变
                    is_get = True
                    return copy.deepcopy(self.program), 0, 0
                continue
            else:
                # 符合条件
                is_get = True
