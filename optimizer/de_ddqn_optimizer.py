...
class DE_DDQN_Optimizer(Learnable_Optimizer):
    ...
    def init_population(self, problem):
        ...
        self.__maxFEs = problem.dim * 2000
    ...
    def __get_state(self, problem):
        ...
        normalizer = self.__c_gworst - self.__c_gbest 
        if normalizer <= 1e-8:
            normalizer = 1e-3
        features[0] = (self.__cost[self.__pointer] - self.__c_gbest) / (normalizer)
        features[1] = (np.mean(self.__cost) - self.__c_gbest) / (normalizer)
        features[2] = np.std(self.__cost) / ((normalizer) / 2)
        ...
        for j in range(0, 5):  # features[12] ~ features[16]
            features[j + 12] = (self.__cost[self.__pointer] - self.__cost[self.__r[j]]) / (normalizer)
        features[17] = (self.__cost[self.__pointer] - self.__c_prebest) / (normalizer)
        ...
        if self.__gen >= 2:
            for op in range(4):
                for m in range(4):
                    ...
                    if self.__N_tot[op][0] - self.__N_tot[op][1] > 1e-8 and self.__N_succ[op][m][0] > 0 and self.__N_succ[op][m][1] > 0 and np.abs(np.max(self.__OM[op][m][1])) > 1e-8:
        ...
        # torch_fea = torch.FloatTensor(features)
        # # 检测 inf 和 nan 的索引
        # if torch.isinf(torch_fea).any():
        #     print("get_state Features with inf at indices:", torch.where(torch.isinf(torch_fea))[0])
        #     raise
        # if torch.isnan(torch_fea).any():
        #     print("get_state Features with nan at indices:", torch.where(torch.isnan(torch_fea))[0])
        #     raise
        return np.clip(features,-1e10,1e10)
