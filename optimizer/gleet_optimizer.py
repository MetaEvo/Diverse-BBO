...
class GLEET_Optimizer(Learnable_Optimizer):
    ...
    def initialize_particles(self, problem):
        ...
        # record
        # 使用预先的最优值进行归一化，避免负数状态
        if self.__config.problem in ['Diverse-BBO','GP_baseline']:
            self.max_cost=np.min(c_cost) - problem.eval_optimum if np.min(c_cost) - problem.eval_optimum > 1e-8 else 1e-3
        else:
            self.max_cost = np.min(c_cost)
        # F158 188 208 223 238 254
        # if self.max_cost == 0:
        # print(f"function {problem.problemID} normalizer {self.max_cost}")
    ...
    def init_population(self, problem):
        ...
        if self.__config.problem not in ['UAV','HPOB','protein']:
            self.__config.maxFEs = problem.dim * 2000
        ...
        if self.w_decay:
            self.w=0.9
        self.max_dist = (problem.ub - problem.lb)**2*self.dim
    ...
    # feature encoding
    def observe(self):
        max_step = self.max_fes // self.ps  # Ensure max_step is not zero to avoid division by zero in fea4 and fea5

        def check_nan_inf(name, arr):
            if np.any(np.isnan(arr)):
                print(f"Feature {name} contains NaN values. Count: {np.sum(np.isnan(arr))}")
                print(f"max_cost :{self.max_cost}")
            if np.any(np.isinf(arr)):
                print(f"Feature {name} contains Inf values. Count: {np.sum(np.isinf(arr))}")
                print(f"max_cost :{self.max_cost}")
                

        # feature0: cost cur
        fea0 = self.particles['c_cost'] / self.max_cost
        check_nan_inf('fea0', fea0)

        # feature1: cost cur_gbest
        fea1 = (self.particles['c_cost'] - self.particles['gbest_val']) / self.max_cost
        check_nan_inf('fea1', fea1)

        # feature2: cost cur_pbest
        fea2 = (self.particles['c_cost'] - self.particles['pbest']) / self.max_cost
        check_nan_inf('fea2', fea2)

        # feature3: fes cur_fes
        fea3 = np.full(shape=(self.ps), fill_value=(self.max_fes - self.fes) / self.max_fes)
        check_nan_inf('fea3', fea3)

        # feature4: no_improve per
        fea4 = self.per_no_improve / max_step
        check_nan_inf('fea4', fea4)

        # feature5: no_improve whole
        fea5 = np.full(shape=(self.ps), fill_value=self.no_improve / max_step)
        check_nan_inf('fea5', fea5)

        # feature6: distance between cur and gbest
        # try:
        ...
        fea6 = np.sqrt(np.sum((self.particles['current_position'] - np.expand_dims(self.particles['gbest_position'], axis=0))**2/self.max_dist, axis=-1))
        # except:
            # print(self.particles['current_position'].shape,self.particles['gbest_position'].shape)
        check_nan_inf('fea6', fea6)

        # feature7: distance between cur and pbest
        # fea7 = np.sqrt(np.sum((self.particles['current_position'] - self.particles['pbest_position'])**2, axis=-1)) / self.max_dist
        fea7 = np.sqrt(np.sum((self.particles['current_position'] - self.particles['pbest_position'])**2/self.max_dist, axis=-1)) 
        check_nan_inf('fea7', fea7)
        ...
        denominator = (np.sqrt(np.sum(pbest_cur_vec**2, axis=-1)) * np.sqrt(np.sum(gbest_cur_vec**2, axis=-1))) + 1e-5
        fea8 = np.sum(pbest_cur_vec * gbest_cur_vec, axis=-1) / denominator
        fea8 = np.where(np.isnan(fea8), np.zeros_like(fea8), fea8)
        check_nan_inf('fea8', fea8)
    ...
    def cal_reward_direct(self,new_gbest,pre_gbest):
        bonus_reward=(pre_gbest-new_gbest)/np.abs(self.max_cost)
