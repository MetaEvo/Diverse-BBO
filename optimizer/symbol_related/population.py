...
class Population(object):
    ...
    def reset(self,init_pop=None,init_y=None,need_his=True):
        ...
        # randomly generate the position and velocity
            rand_pos=self.rng.uniform(low=self.min_x,high=self.max_x,size=(self.pop_size,int(self.dim)))
        else:
            rand_pos=init_pop
        ...
        # record
        # 此处应该修改
        if self.problem.optimum is None and hasattr(self.problem, 'eval_optimum'):
            self.init_cost = np.min(self.c_cost) - self.problem.eval_optimum if  np.min(self.c_cost) - self.problem.eval_optimum > 1e-8 else 1e-3
            # if np.min(self.c_cost) - self.problem.eval_optimum <= 1e-8:
            #     print(f"functionID {self.problem.problemID}")
        else:
    ...
    def feature_encoding(self):
        ...
        normalizer = self.gworst_cost-self.gbest_cost
        if normalizer < 1e-8:
            normalizer = 1e-3
        fea_1=(self.c_cost-self.gbest_cost)/(normalizer+1e-8)
        fea_1=np.mean(fea_1)
        
        # fea_2=calculate_mean_distance(self.current_position)/self.max_dist
        fea_2=calculate_mean_distance(self.current_position,self.max_dist,self.dim)

        fit=np.zeros_like(self.c_cost)
        fit[:self.pop_size//2]=self.gworst_cost
        fit[self.pop_size//2:]=self.gbest_cost
        maxstd=np.std(fit)
        if np.abs(maxstd) < 1e-8:
            maxstd = 1e-3
        ...
        fea_6=dist(self.current_position/self.max_dist,self.cbest_position[None,:]/self.max_dist)/np.sqrt(self.dim)
        fea_6=np.mean(fea_6)

        fea_7=(self.c_cost-self.cbest_cost)/(normalizer+1e-8)
        fea_7=np.mean(fea_7)

        fea_8=dist(self.current_position/self.max_dist,self.gbest_position[None,:]/self.max_dist)/np.sqrt(self.dim)
        fea_8=np.mean(fea_8)
        ...
        if np.isnan(self.c_cost).any():
            print(self.c_cost)
            print(self.current_position[np.isnan(self.c_cost)][0])
            print(f"func {self.problem.problemID}")

        assert not np.any(np.isnan(feature)),f'feature has nan!!,{feature}'
        assert not np.any(np.isinf(feature)),f'feature has inf!!,{feature}'
        return feature
    
def calculate_mean_distance(population,max_dist,dim):
    distances = cdist(population/max_dist, population/max_dist, metric='euclidean')
    
    np.fill_diagonal(distances, 0)
    distances = distances/np.sqrt(dim)
