...
class MadDE():
    ...
    def init_population(self, problem,rng):
    ...
    def update(self, action):
        ...
        while not step_end:
            ...
            # 下面计算count_S的时候，用到self.population.c_cost的绝对取值进行归一化
            # 需要减去optimum，并且非0处理
            if self.config.problem in ['Diverse-BBO','GP_baseline']:
                normalizer_c_cost = np.maximum(self.population.c_cost - self.problem.eval_optimum, 1e-3)
            else:
                normalizer_c_cost = self.population.c_cost
            
            for i in range(3):
                count_S[i] = np.mean(df[mu == i] / normalizer_c_cost[mu == i])
            if np.sum(count_S) > 0:
            ...
            # check if __pm contains NaN
            # if np.max(np.abs(self.population.c_cost)) > 1e15:
            #     print(f"big eval : {self.problem.problemID}")
            assert not np.any(np.isnan(self.__pm)), f"self.__pm is NaN! Values: {self.__pm}, count_S: {count_S}, df: {df}, c_cost: {self.population.c_cost} func{self.problem.problemID}"
