...
def rand_1(x: np.ndarray, F: Union[np.ndarray, float], rng: np.random.RandomState = None) -> np.ndarray:
    """
...
def rand_2(x: np.ndarray, F: Union[np.ndarray, float], rng: np.random.RandomState = None) -> np.ndarray:
    """
    :param x: The 2-D population matrix of shape [NP, dim].
    :param F: The mutation factor, which could be a float or a 1-D array of shape[NP].
    """
    if isinstance(F, np.ndarray) and F.ndim == 1:
        F = F.reshape(-1, 1)
    r = generate_random_int(x.shape[0], 5, rng=rng)
    return x[r[:, 0]] + F * (x[r[:, 1]] - x[r[:, 2]] + x[r[:, 3]] - x[r[:, 4]])


def best_1_single(x: np.ndarray, best: np.ndarray, F: float, pointer: int, r: np.ndarray = None, rng: np.random.RandomState = None) -> np.ndarray:
    if r is None:
        r = generate_random_int_single(x.shape[0], 2, pointer, rng=rng)
    return best + F * (x[r[0]] - x[r[1]])


def best_1(x: np.ndarray, best: np.ndarray, F: Union[np.ndarray, float], rng: np.random.RandomState = None) -> np.ndarray:
    """
    :param x: The 2-D population matrix of shape [NP, dim].
...
def best_2(x: np.ndarray, best: np.ndarray, F: Union[np.ndarray, float], rng: np.random.RandomState = None) -> np.ndarray:
    """
    :param x: The 2-D population matrix of shape [NP, dim].
    :param best: An array of the best individual of shape [dim].
    :param F: The mutation factor, which could be a float or a 1-D array of shape[NP].
    ...
    if isinstance(F, np.ndarray) and F.ndim == 1:
        F = F.reshape(-1, 1)
    r = generate_random_int(x.shape[0], 4, rng=rng)
    return best + F * (x[r[:, 0]] - x[r[:, 1]] + x[r[:, 2]] - x[r[:, 3]])


def rand_to_best_1_single(x: np.ndarray, best: np.ndarray, F: float, pointer: int, r: np.ndarray = None, rng: np.random.RandomState = None) -> np.ndarray:
    if r is None:
        r = generate_random_int_single(x.shape[0], 3, pointer, rng=rng)
    return x[r[0]] + F * (best - x[r[0]] + x[r[1]] - x[r[2]])


def rand_to_best_1(x: np.ndarray, best: np.ndarray, F: Union[np.ndarray, float], rng: np.random.RandomState = None) -> np.ndarray:
    """
    :param x: The 2-D population matrix of shape [NP, dim].
    :param best: An array of the best individual of shape [dim].
    :param F: The mutation factor, which could be a float or a 1-D array of shape[NP].
...
def rand_to_best_2(x: np.ndarray, best: np.ndarray, F: Union[np.ndarray, float], rng: np.random.RandomState = None) -> np.ndarray:
    """
    :param x: The 2-D population matrix of shape [NP, dim].
    :param best: An array of the best individual of shape [dim].
    :param F: The mutation factor, which could be a float or a 1-D array of shape[NP].
...
def cur_to_best_1_single(x: np.ndarray, best: np.ndarray, F: float, pointer: int, r: np.ndarray = None, rng: np.random.RandomState = None) -> np.ndarray:
    if r is None:
        r = generate_random_int_single(x.shape[0], 2, pointer, rng=rng)
...
def cur_to_best_2_single(x: np.ndarray, best: np.ndarray, F: float, pointer: int, r: np.ndarray = None, rng: np.random.RandomState = None) -> np.ndarray:
    if r is None:
        r = generate_random_int_single(x.shape[0], 4, pointer, rng=rng)
    return x[pointer] + F * (best - x[pointer] + x[r[0]] - x[r[1]] + x[r[2]] - x[r[3]])


def cur_to_best_2(x: np.ndarray, best: np.ndarray, F: Union[np.ndarray, float], rng: np.random.RandomState = None) -> np.ndarray:
    """
    :param x: The 2-D population matrix of shape [NP, dim].
    :param best: An array of the best individual of shape [dim].
    :param F: The mutation factor, which could be a float or a 1-D array of shape[NP].
...
def cur_to_rand_2_single(x: np.ndarray, F: float, pointer: int, r: np.ndarray = None, rng: np.random.RandomState = None) -> np.ndarray:
    if r is None:
        r = generate_random_int_single(x.shape[0], 5, pointer, rng=rng)
    return x[pointer] + F * (x[r[0]] - x[pointer] + x[r[1]] - x[r[2]] + x[r[3]] - x[r[4]])


def cur_to_rand_2(x: np.ndarray, F: Union[np.ndarray, float], rng: np.random.RandomState = None) -> np.ndarray:
    if isinstance(F, np.ndarray) and F.ndim == 1:
        F = F.reshape(-1, 1)
    r = generate_random_int(x.shape[0], 5, rng=rng)
