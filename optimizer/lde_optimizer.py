...
class LDE_Optimizer(Learnable_Optimizer):
    ...
    def init_population(self, problem):
        # 根据问题来设定dim和maxFES
        self.__config.dim = int(problem.dim)
    ...
    def update(self, action, problem):
        ...
        # 初始化奖励数组
        reward = np.zeros_like(bsf, dtype=np.float32)
        # 找到 (bsf - problem.episode_optimum) > 1e-8 的索引
        if problem.optimum is None and self.__config.problem in ['Diverse-BBO','GP_baseline']:
            non_zero_mask = (bsf - problem.episode_optimum) > 1e-8
             # 按照公式计算奖励
            reward[non_zero_mask] = (bsf[non_zero_mask] - bsf_next[non_zero_mask]) / (bsf[non_zero_mask]- problem.episode_optimum)
            zero_mask = ~non_zero_mask
            reward[zero_mask] = np.where(bsf[zero_mask] - bsf_next[zero_mask] > 0,1.,\
            np.where(bsf[zero_mask] - bsf_next[zero_mask]<0,-1.,0))
        elif problem.optimum is not None:
            non_zero_mask = (bsf - problem.optimum) > 1e-8
            # 按照公式计算奖励
            reward[non_zero_mask] = (bsf[non_zero_mask] - bsf_next[non_zero_mask]) / (bsf[non_zero_mask]- problem.optimum)
            zero_mask = ~non_zero_mask
            reward[zero_mask] = np.where(bsf[zero_mask] - bsf_next[zero_mask] > 0,1.,\
                np.where(bsf[zero_mask] - bsf_next[zero_mask]<0,-1.,0))
        else:
            reward = (bsf- bsf_next)/ np.abs(bsf)
        # 对 bsf - problem.episode_optimum <= 1e-8 的情况，
        # 可能是bsf得到了比optimum更小的值；或者bsf已经达到了optimum
        # 此时难以归一化 直接根据是否进步来进行reward 进步给1 退步给-1,没进步给0
        ...
        # 需要更新episode_optimum保证下次reward更新
        if problem.optimum is None and self.__config.problem in ['Diverse-BBO','GP_baseline'] and np.min(self.__fit) < problem.episode_optimum:
            problem.episode_optimum = np.min(self.__fit)
