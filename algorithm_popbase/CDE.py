...
class CDE(DE):
    ...
    def select(self, v=None, x=None, y=None, args=None):
        ...
        #         x[i], y[i] = v[i], yy
        # 改写成数组操作
        yy = self._evaluate_fitness(v, args)
        # 将新yy和原y对比，取最小值，同时更新对应index的x
        mask = yy < y
        x[mask] = v[mask]
        y[mask] = yy[mask]
        return x, y
    ...
    def iterate(self, x=None, y=None, v=None, args=None):
        ...
        # 交叉变异之后需要边界处理
        if self.is_bound:
            v = np.clip(v, self.lower_boundary, self.upper_boundary)
        x, y = self.select(v, x, y, args)
        self._n_generations += 1
        if self.n_function_evaluations % self.log_interval == 0:
            self.log_index += 1
