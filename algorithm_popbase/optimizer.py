...
class Optimizer(object):
    ...
    def _evaluate_fitness(self, x, args=None):
        ...
        self.n_function_evaluations += x.shape[0]
        # update best-so-far solution (x) and fitness (y)
        
        if np.min(y) < self.best_so_far_y:
            min_idx = np.argmin(y)
            self.best_so_far_x, self.best_so_far_y = np.copy(x[min_idx]), y[min_idx]
            # self.best_so_far_x, self.best_so_far_y = np.copy(x), y
        # update all settings related to early stopping
        if (self._base_early_stopping - np.min(y)) <= self.early_stopping_threshold:
            self._counter_early_stopping += 1
        else:
            self._counter_early_stopping, self._base_early_stopping = 0, np.min(y)
        # return float(y)
        return y
