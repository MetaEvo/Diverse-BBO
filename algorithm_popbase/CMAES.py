...
class CMAES(ES):
    ...
    .. note:: `CMAES` is widely recognized as one of **State-Of-The-Art (SOTA)** evolutionary algorithms for continuous
       ...
       <https://ieeexplore.ieee.org/abstract/document/10531788>`_, `[Falk et al., 2023, PNAS]
    ...
    def __init__(self, problem, options):
        ...
        self.log_points = options.get('log_points', 10000) 
        self.log_index = 0
        self.log_interval = self.max_function_evaluations // self.log_points
        self.records = []  # 用于存储记录的列表
        
    def bound(self, x=None):
        if not self.is_bound:
            return x
        idx = np.array(x < self.lower_boundary)
        if idx.any():
            x[idx] = self.lower_boundary[idx]
        idx = np.array(x > self.upper_boundary)
        if idx.any():
            x[idx] = self.upper_boundary[idx]
        return x
    ...
    def iterate(self, x=None, mean=None, e_ve=None, e_va=None, y=None, d=None, args=None):
        ...
        # 向量化运行处理
        # 一次性生成所有个体的随机噪声
        z = self.rng_optimization.standard_normal((self.n_individuals, self.ndim_problem))  # (n_individuals, ndim_problem)
        d = np.dot(e_ve @ np.diag(e_va), z.T).T  # (n_individuals, ndim_problem)
        # 生成所有个体
        x = mean + self.sigma * d  # (n_individuals, ndim_problem)
        # 边界处理
        if self.is_bound:
            x = np.clip(x, self.lower_boundary, self.upper_boundary)
        # 一次性评估所有个体的适应度
        y = self._evaluate_fitness(x, args)  # (n_individuals,)
        if self.n_function_evaluations % self.log_interval == 0 or self.log_index == 0:
            self.log_index += 1
            # self.records.append({'log_points': self.log_index,'best_x': self.best_so_far_x,'best_y': self.best_so_far_y, 'fes': self.n_function_evaluations})
            self.records.append({'log_points': self.log_index,'best_y': self.best_so_far_y})
        return x, y, d
    ...
    def optimize(self, fitness_function=None, args=None):  # for all generations (iterations)
        ...
        results['log_points'] = self.records  # 将记录添加到结果中
        # return results
        return {'log_points' : self.records,
