    ...
    def __init__(self, config):
        ...
        if agent_name is not None:  # learnable optimizer
            ...
            with open(file_path, 'rb') as f:
                ...
                # self.agent.test_run = config.test_run
        ...
        if self.config.all_instances:
            self.test_set= construct_problem_set(self.config)
        else:
        ...
        with tqdm(range(pbar_len), desc = "Testing") as pbar:
            for i, problem in enumerate(self.test_set.data):
                for agent_id, (agent, optimizer) in enumerate(zip(self.agent_for_cp, self.l_optimizer_for_cp)):
                    ...
                    self.test_results['cost'][problem.__str__()][self.agent_name_list[agent_id]].extend(meta_test_data['cost'])
                    self.test_results['fes'][problem.__str__()][self.agent_name_list[agent_id]].extend(meta_test_data['fes'])
