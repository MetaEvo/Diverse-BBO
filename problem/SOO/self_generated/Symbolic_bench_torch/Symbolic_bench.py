...
from .functions_torch import _Function, _protected_sub, _sum, _prod, _mean, _sigmoid, _protected_power,_tanh
from .functions_torch import _protected_division, _protected_sqrt, _protected_log, _protected_inverse, _protected_exp 
...
class _Program(object):
    ...
    def execute(self, X):  # X是一个由多个输入向量组成的矩阵
        ...
        if isinstance(node, list):  # 常数向量检测，检测np.ndarray类型
            print('constant')
            return torch.repeat_interleave(node[0][0], X.size(0))  # 对每个输入向量返回一个实数
        ...
        for index, node in enumerate(self.program):
            ...
            while len(apply_stack[-1]) == apply_stack[-1][0].arity + 1:  # 操作数凑齐时开始计算
                    ...
                    if isinstance(t, list):  # 常数向量改为list[ndarray]类型，避免了后续的混淆
                        ...
                        for item in apply_stack[-1][1:]:
                            if isinstance(item, tuple):  # 变量节点
                                length = X.size(1) - item[1] - item[0]
                                # length = item[1] - item[0]
                                break
                            elif isinstance(item, torch.Tensor):
                                # print(item.shape)
                                if len(item.size()) > 1:
                                    length = item.size(0)  # dimension维度
                                else:
                                    length = 1
                                break
                        assert (length > 0)
                        ...
                        # torch计算要全部换成tensor 下面直接扩展维度 就不需要list包着了
                        t = t[0].reshape(1,-1)
                        temp = torch.repeat_interleave(t, X.size(0),dim = 0)  # n_samples x dimension of t
                        # 调整维度顺序，n_samples调整为最后一维，因为sum和prod等aggregate函数是按axis=0来进行计算的
                        # print(temp.size())
                        temp = torch.permute(temp,(1,0))  # dimension x n_samples
                        # print(f'const3:{temp.shape}', end='\n\n')
                        terminals.append(temp)
                    elif isinstance(t, tuple):
                        # t[1]表示从右开始数到切片末尾所需的次数
                        temp = X[:, t[0]:X.size(1) - t[1]]  # n_samples x dimension of t
                        # 调整维度顺序，n_samples调整为最后一维，因为sum和prod等aggregate函数是按axis=0来进行计算的
                ...
                if function.name in ['sum', 'prod', 'mean']:
                    ...
                    terminals = torch.stack(terminals)  # 默认沿第 0 维堆叠
                    # terminals = torch.tensor(terminals)
                    # arity>1时sum和prod保持输入和输出维度相同，arity=1时输入为向量，输出为实数
                    if terminals.ndim > 2 and terminals.size(0) == 1:
    ...
    def _generate_eval_expression(self, x,index,dimension = None):
        ...
        if isinstance(node, list):  # 常数向量节点
            ...
            if dimension == self.n_features - 1:
                self.constants[const_name] = torch.tensor(node[0][:self.best_dim-1])
                self.constants[const_name] = torch.repeat_interleave([self.constants[const_name]],x.shape[0],dim=0)
            elif dimension == self.n_features:
                self.constants[const_name] = torch.tensor(node[0][:self.best_dim])
                self.constants[const_name] = torch.repeat_interleave([self.constants[const_name]],x.shape[0],dim=0)
            elif dimension == 1:
                # 此时就是聚合维度
                self.constants[const_name] =  torch.tensor(node[0])
            else:
                print(f"dimension error: {dimension},  constance index {index} , node {node[0]}")
            # print(self.constants[const_name].shape)
            return f' torch.tensor({self.constants[const_name].tolist()})'
        ...
        if isinstance(node, _Function):  # 函数节点
            ...
            node_name = node.name
            if node_name == 'mul':
                node_name = 'torch.multiply'
            elif node_name == 'neg':
                node_name = 'torch.negative'
            elif node_name == 'abs':
                node_name = 'torch.abs'
            elif node_name == 'sin':
                node_name = 'torch.sin'
            ...
            if node_name in ['sum','mean']:
                if node.arity == 1:
                    ...
                    # 需要执行对最后一个维度的聚合
                    if node_name == 'sum':
                        return f"torch.sum({', '.join(child_expressions)},dim = -1)"
                    else:
                        return f"torch.mean({', '.join(child_expressions)},dim = -1)"
                else:
                    ...
                    child_expressions = f"torch.stack([{', '.join(child_expressions)}])"
                    if node_name == 'sum':
    ...
    def _generate_latex_expression(self, index):
        ...
        if isinstance(node, list):  # 常数向量节点
            ...
            if torch.tensor(node[0]).size(-1) == 1:
                const_name = f"{node[0][0]}"
            else:
                const_name = f"C{len_index+1}"
                self.latex_constants[const_name] = torch.tensor(node[0])
            return const_name
        ...
        if isinstance(node, tuple):  # 变量节点
            ...
            if a == 0:
                ...
                var_name = "X2"
            self.latex_variables[var_name] = f"x[:, {a}:x.size(1)-{b}:{c}]"
            return var_name
    ...
    def replace_functions_and_tensor(self):
        ...
        for item in self.program:
            if isinstance(item, tuple):  # 变量向量
                ...
                # 此时list内的是ndarray需要换成tensor
                item = [torch.tensor(item[0],dtype=torch.float64)]
                # print(item)
                # print(type(item))
                # print(type(item[0]))
                new_program.append(item)
