...
def calculate_ela_meta(
      ...
      # t1 = time.monotonic()
      # print(f"pure linear :{t1 - start_time}")
      ...
      # t2 = time.monotonic()
      # print(f"linear with_interact:{t2 - t1}")
...
def calculate_ela_conv(
      ...
      for _ in range(ela_conv_nsample):
            ...
            # 归一化计算的f 和 线性组合的y
            delta.append((f(xn)-y.min())/(y.max()-y.min()) - np.matmul(((y-y.min())/(y.max()-y.min())).iloc[i], wt))
            nfev += 1
...
def calculate_ela_level(
      ...
      # lda_qda = np.array([lda_mmce[i]/qda_mmce[i] if qda_mmce[i] != 0 else -1 for i in range(len(ela_level_quantiles))])
      if interface_mda_from_R:
