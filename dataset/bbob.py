...
class BBOB_Dataset(Dataset):
    ...
    def get_datasets(suit,
        ...
        if suit == 'bbob':
            ...
            small_set_func_id = [3,4,6,8,9,10,11,12,14,15,19,20]
...
# To provide the random seed issue when we run code on Ray cluster, we generate our problem set here and save it
# if __name__ == '__main__':
#     import pickle
#     tr,te = BBOB_Dataset.get_datasets()
...
if __name__ == '__main__':
    import pickle
    tr,te = BBOB_Dataset.get_datasets(suit='bbob-noisy', shifted=True)
    print(len(tr), len(te))
    with open('./noisy_trainset.pkl','wb') as f:
        pickle.dump(tr,f)
    with open('./noisy_testset.pkl','wb') as f:
        pickle.dump(te,f)